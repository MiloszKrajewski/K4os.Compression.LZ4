...
public partial class LZ4Stream: Stream
{
	...
	public enum ChunkFlags
	{
		...
		None = 0x00, /// <summary>Set if chunk is compressed.</summary>
		Compressed = 0x01,
	...
	private void FlushCurrentChunk()
	{
		...
		var compressedLength = LZ4Codec.Encode(
			_buffer, 0, _bufferOffset,
			compressed, 0, _bufferOffset,
			compressionLevel);
		...
		do
		{
			...
			else
			{
				...
				var decodedLength = LZ4Codec.Decode(
					...
					_buffer, 0, originalLength);
				if (decodedLength != originalLength)
					throw new InvalidDataException("Compressed data corrupted");

...
namespace K4os.Compression.LZ4.Streams
{
	public partial class LZ4DecoderStream
	{
		...
		private Task InnerFlush(CancellationToken token) =>
			_inner.FlushAsync(token);

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private Task<int> InnerRead(
			CancellationToken token, byte[] buffer, int offset, int length) =>
			_inner.ReadAsync(buffer, offset, length, token);
		
		private async Task<int> PeekN(
			CancellationToken token, byte[] buffer, int offset, int count, bool optional = false)
		{
			var index = 0;
			while (count > 0)
			{
		...
		private async Task<bool> EnsureFrame(CancellationToken token) => 
			_decoder != null || await ReadFrame(token);

		[SuppressMessage("ReSharper", "InconsistentNaming")]
		private async Task<bool> ReadFrame(CancellationToken token)
		{
			FlushPeek();
		...
		private async Task<int> ReadImpl(
			CancellationToken token, byte[] buffer, int offset, int count)
		{

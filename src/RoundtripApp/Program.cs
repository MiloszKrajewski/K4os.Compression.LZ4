...
static void Roundtrip(string temp, string filename, LZ4Level level)
{
	...
	using (var targetFile = LZ4Stream.Encode(File.Create(encodedName), level, Mem.M1))
	{
	...
	using (var sourceFile = LZ4Stream.Decode(File.OpenRead(encodedName), Mem.M1))
...
static uint Checksum(Stream file)
{
	var hash = new XXH32();
